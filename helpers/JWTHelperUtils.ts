...
import { STATUS_CODES, JSON_WEB_TOKEN_ERROR } from "../constants/constants";
const JWT = require("jsonwebtoken");

module.exports = {
  getAccessToken: (userId: string) => {
    const payload: JwtPayload = {};
    const secretKey: string | undefined = process.env.BCRYPT_SECRET_KEY;
  ...
  verifyAccessToken: (req: Request, res: Response, next: NextFunction) => {
    ...
    JWT.verify(
      ...
      (err: Error, payload: JwtPayload) => {
        if (err) {
          const message =
            err.name === JSON_WEB_TOKEN_ERROR ? "Unauthorized" : err.message;
          const status =
            err.name === JSON_WEB_TOKEN_ERROR
  ...
  getRefreshToken: (userId: string) => {
    ...
    const secretKey: string | undefined
     = process.env.BCRYPT_REFRESH_TOKEN_SECRET_KEY;
    const options: SignOptions = {
      expiresIn: "1y",
      audience: String(userId), //We need to convert to string since id from database will be of type number
    };
  ...
  verifyRefreshToken: (refreshToken: string) => {
    ...
    return new Promise((resolve, reject) => {
      JWT.verify(
        refreshToken,
        secretKey,
        function (err: Error, payload: JwtPayload) {
          if (err) {
            return reject(err);
          }
